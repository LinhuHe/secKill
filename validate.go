...
func GetDataFromOtherMap(host string, request *http.Request) bool {
	...
	//获取sign
	uidSign, err := request.Cookie("uidEncode")
	if err != nil {
		return false
	}
	...
	//手动指定，排查多余cookies
	cookieUid := &http.Cookie{Name: "uid", Value: uidPre.Value, Path: "/", Domain: "*"}
	cookieSign := &http.Cookie{Name: "uidEncode", Value: uidSign.Value, Path: "/", Domain: "*"}
	//添加cookie到模拟的请求中
...
func CheckUserInfo(r *http.Request) error {
	...
	//获取用户加密串
	signCookie, err := r.Cookie("uidEncode")
	if err != nil {
		//return errors.New("用户加密串 Cookie 获取失败！")
	}

	//对信息进行解密
	signByte, err := tool.DePwdCode(signCookie.Value)
	if err != nil {
